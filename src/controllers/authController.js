...
export const authController = {
  registerAccount: async (req, res) => {
    try {
      ...
      console.log(
        `{process.env.APP_URL}/api/v1/auth/verify?email=${email}&token=${encryptedEmail}`
      );
      sendMail(
        email,
        "Verify Email",
        `<a href="${process.env.APP_URL}/api/v1/auth/verify?email=${email}&token=${encryptedEmail}"> Verify </a>`
      );
  ...
  verify: async (req, res) => {
    console.log(decrypt(req.query.token));
